...
import { AutomationHistory } from './AutomationHistory'; // <-- Nuestra nueva integración
import { 
  Zap, ArrowRight, BellRing, PackageCheck, CheckCircle2, Bot, 
  Power, PowerOff, Plus, ChevronDown, ChevronUp, Save, Trash2, 
  Mail, MessageCircle, Settings2, History 
} from 'lucide-react';
...
export default function AutomationsIndex() {
  ...
  return (
    <AdminLayout title="Motor de Automatizaciones" subtitle="Configura canales de notificación automáticos y supervisa los envíos">
      <div className="ff-page-wrapper">
        
        {/* NAVEGACIÓN POR PESTAÑAS (CSS PURO) */}
        <div className="tab-navigation">
          <button 
            onClick={() => setActiveTab('rules')}
            className={`tab-item ${activeTab === 'rules' ? 'active' : ''}`}
          >
            <Settings2 size={16} /> Configuración de Reglas
          </button>
          <button 
            onClick={() => { setActiveTab('history'); setIsCreating(false); }}
            className={`tab-item ${activeTab === 'history' ? 'active' : ''}`}
          >
            <History size={16} /> Auditoría y Logs
          </button>
        </div>
      ...
      <style>{`
        .ff-page-wrapper { display: flex; flex-direction: column; font-family: 'Poppins', sans-serif; padding-bottom: 40px; }
        
        /* PESTAÑAS (TABS) - CSS PURO */
        .tab-navigation {
          display: flex;
          background: #e2e8f0;
          padding: 6px;
        ...
        .tab-item:hover:not(.active) { color: #475569; }
        .tab-item.active { background: white; color: #224c22; box-shadow: 0 2px 8px rgba(0,0,0,0.05); }
        .tab-content-fade { display: flex; flex-direction: column; gap: 24px; animation: fadeIn 0.4s ease-out; }

        /* ESTILOS EXISTENTES */
        .engine-status-banner { display: flex; align-items: center; justify-content: space-between; gap: 20px; background: linear-gradient(135deg, var(--ff-green-dark, #224c22) 0%, #1a3a1a 100%); padding: 24px; border-radius: 20px; color: white; }
        .esb-icon { width: 60px; height: 60px; background: rgba(255,255,255,0.1); border-radius: 16px; display: flex; align-items: center; justify-content: center; color: var(--ff-orange, #f97316); }
        .esb-text { flex: 1; }
        ...
        .btn-create-rule { background: var(--ff-orange, #f97316); color: white; border: none; padding: 12px 20px; border-radius: 12px; font-weight: 700; display: flex; align-items: center; gap: 8px; cursor: pointer; transition: 0.2s; }
        ...
        .rule-builder-card h4 { color: var(--ff-green-dark, #224c22); margin: 0 0 20px 0; font-size: 18px; }
        ...
        .btn-add-step { background: none; border: none; color: var(--ff-orange, #f97316); font-weight: 700; font-size: 13px; cursor: pointer; padding: 5px 0; margin-bottom: 20px; margin-left: 42px; }
        .builder-footer { border-top: 1px solid #f1f5f9; padding-top: 20px; display: flex; justify-content: flex-end; }
        .btn-save-rule { background: var(--ff-green-dark, #224c22); color: white; border: none; padding: 12px 24px; border-radius: 10px; font-weight: 700; display: flex; align-items: center; gap: 8px; cursor: pointer; }
        ...
        .ac-header h4 { margin: 0; font-size: 15px; color: var(--ff-green-dark, #224c22); }
        ...
        .workflow-title { font-size: 11px; font-weight: 800; color: var(--ff-green-dark, #224c22); margin-bottom: 16px; display: flex; align-items: center; gap: 6px; }
        ...
        .step-dot { width: 12px; height: 12px; border-radius: 50%; background: white; border: 2.5px solid var(--ff-orange, #f97316); margin-top: 4px; box-shadow: 0 0 0 4px #f8fafc; }
        .step-content { flex: 1; }
        .step-header { display: flex; align-items: center; gap: 12px; margin-bottom: 4px; }
        .step-role { font-size: 13px; font-weight: 700; color: var(--ff-green-dark, #224c22); }
